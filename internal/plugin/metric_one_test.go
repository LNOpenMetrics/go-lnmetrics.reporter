...
func TestJSONMigrationFrom0to1(t *testing.T) {
	jsonString := `{
   "channels_info": {"fake":
{
         "capacity": 450000,
...
func TestJSONMigrationFrom0to1Failure(t *testing.T) {
	jsonString := `{
   "channels_info": {"fake":
{
         "capacity": 450000,
...
"up_times": []
   ...
   "up_time": [
	...
	var metric MetricOne
	panicF := func() {
		json.Unmarshal([]byte(jsonString), &metric)
	}
	assert.Panics(t, panicF, "No error received from the migration phase from version 0 to 1")
}
